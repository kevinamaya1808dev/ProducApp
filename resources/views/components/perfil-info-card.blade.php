...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div class="px-6 pb-6 relative">
        ...
        <h2 class="text-xl font-bold text-slate-800">{{ $usuario['nombre'] ?? 'Sin nombre' }}</h2>
        <p class="text-slate-500 text-sm mb-6">{{ $usuario['puesto'] ?? 'Operario' }}</p>
        
        <ul class="space-y-3 text-sm">
            <li class="flex justify-between border-b border-slate-50 pb-2">
                <span class="text-slate-400 font-medium">ID</span>
                <span class="font-bold text-slate-800">{{ $usuario['id_operario'] ?? '—' }}</span>
            </li>
            <li class="flex justify-between border-b border-slate-50 pb-2">
                <span class="text-slate-400 font-medium">ESTACIÓN</span>
                <span class="font-medium text-slate-800">{{ $usuario['estacion'] ?? 'Sin asignar' }}</span>
            </li>
            <li class="flex justify-between border-b border-slate-50 pb-2">
                <span class="text-slate-400 font-medium">TURNO</span>
                <span class="font-medium text-slate-800">{{ $usuario['turno'] ?? 'Sin definir' }}</span>
            </li>
            <li class="flex justify-between border-b border-slate-50 pb-2">
                <span class="text-slate-400 font-medium">PLANTA</span>
                <span class="font-medium text-slate-800">{{ $usuario['planta'] ?? 'Sin definir' }}</span>
            </li>
            <li class="flex justify-between pt-1">
                <span class="text-slate-400 font-medium">ALTA DESDE</span>
                <span class="font-medium text-slate-800">{{ $usuario['alta_desde'] ?? '—' }}</span>
            </li>
        </ul>
    </div>
</div>
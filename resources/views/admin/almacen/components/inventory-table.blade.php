                ...
                @forelse($materials as $mat)
                    <tr class="{{ $mat->stock_actual <= $mat->stock_minimo ? 'bg-amber-50/50 dark:bg-amber-950/20' : '' }}">
                        ...
                        <td class="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                            {{-- NUEVO: Agregar Stock (suma, no reemplaza) --}}
                            <button @click="activeMaterial = {{ json_encode($mat) }}; openAddStockModal = true" class="text-emerald-600 hover:text-emerald-800 dark:text-emerald-400 dark:hover:text-emerald-300 font-medium text-xs cursor-pointer inline-flex items-center gap-1">

...
>
    ...
    <div class="flex justify-between items-start mb-4">
        <div class="flex items-center gap-3">
            <div class="w-12 h-12 rounded-full bg-orange-600 dark:bg-orange-700 text-white font-bold flex items-center justify-center text-sm shadow-sm">{{ $initials }}</div>
            <div>
                <h3 class="font-bold text-slate-900 dark:text-stone-100 text-base group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">{{ $user->name }}</h3>
                <span class="inline-block bg-orange-50 dark:bg-orange-950/50 text-orange-700 dark:text-orange-400 border border-orange-100 dark:border-orange-900/50 text-[10px] font-bold px-2 py-0.5 rounded-md mt-0.5">{{ $role?->name ?? 'Sin rol' }}</span>
            </div>
        </div>
    </div>
    <div class="pt-3 border-t border-slate-100 dark:border-stone-800 text-xs">
        <p class="text-slate-400 dark:text-stone-500 font-medium uppercase tracking-wider text-[10px]">Correo</p>
        <p class="font-bold text-slate-800 dark:text-stone-200 mt-0.5 truncate">{{ $user->email }}</p>
    </div>
</div>
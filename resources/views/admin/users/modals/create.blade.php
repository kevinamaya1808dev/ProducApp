        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl w-full border border-slate-200">
            <form action="{{ route('admin.users.store') }}" method="POST">
                ...
                <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h3 class="text-lg font-bold text-slate-900">Crear Nuevo Operario</h3>
                    <button type="button" onclick="closeCreateModal()" aria-label="Cerrar modal" class="text-slate-400 hover:text-slate-600">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="name" required value="{{ old('name') }}" class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-orange-500" placeholder="Ej. Juan Pérez">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-1">Rol <span class="text-red-500">*</span></label>
                                <select name="role_id" required class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-orange-500">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                            <div>
                                ...
                                <input type="email" name="email" required value="{{ old('email') }}" class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-orange-500" placeholder="correo@producapp.com">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-1">Estado de la cuenta <span class="text-red-500">*</span></label>
                                <select name="active" required class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-orange-500">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                            <div>
                                ...
                                <select name="turno" class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-orange-500">
                            ...
                            <div>
                                ...
                                <input type="text" name="planta" placeholder="Ej. Estación 1" class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-orange-500">
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="notas" rows="2" placeholder="Habilidades, observaciones..." class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-orange-500"></textarea>
                        </div>

                        <!-- Fila 5: Contraseña -->
                        <div class="mt-4 pt-4 border-t border-slate-100">
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Contraseña <span class="text-red-500">*</span></label>
                            <input type="password" name="password" required placeholder="Mínimo 8 caracteres" minlength="8" autocomplete="new-password" class="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-orange-500">
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
                            @foreach(['Ensamblaje', 'Control de Calidad', 'Soldadura', 'Empaque y Etiquetado', 'Mantenimiento Preventivo', 'Operación de CNC'] as $skillOption)
                                <label class="flex items-center gap-3 p-2 bg-white rounded-lg border border-slate-200 cursor-pointer">
                                    <input type="checkbox" name="skills[]" value="{{ $skillOption }}" class="w-4 h-4 text-orange-600 rounded border-slate-300">
                    ...
                    <div class="border-t border-slate-100 pt-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
                            @forelse($permissions as $permission)
                                <label class="flex items-center gap-3 p-2 bg-white rounded-lg border border-slate-200 cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="w-4 h-4 text-orange-600 rounded border-slate-300">
                ...
                <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                    <button type="button" onclick="closeCreateModal()" class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">Cancelar</button>

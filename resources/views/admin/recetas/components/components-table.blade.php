    ...
    <div class="overflow-x-auto border border-slate-200/80 rounded-xl">
                    ...
                    <tr class="hover:bg-slate-50/60 transition-colors group">
                        ...
                        <td class="py-3 px-4">
                            <div class="flex items-center justify-end gap-1">
                                <button type="button" onclick="openModal('editComponentModal-{{ $component->id }}')" class="p-1.5 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors">
        ...
        @if($recipe->components->isNotEmpty())
            <div class="flex items-center justify-between px-4 py-3 bg-slate-50/50 border-t border-slate-100 flex-wrap gap-2">
                ...
                <span class="text-xs text-slate-400">{{ $recipe->components->count() }} componentes</span>
            </div>
        @endif
    </div>
</div>
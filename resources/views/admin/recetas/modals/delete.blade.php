        ...
        <form action="{{ route('recipes.destroy', $recipe->id) }}" method="POST">
            ...
            <div class="p-6 space-y-3">
                <p class="text-sm text-slate-600 dark:text-stone-300">
                    ¿Estás seguro de que deseas eliminar la receta <strong class="text-slate-900 dark:text-stone-100">{{ $recipe->name }}</strong>?
                </p>
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-stone-800/50 border-t border-slate-100 dark:border-stone-800 flex justify-end gap-2">
                <button type="button" onclick="closeModal('deleteRecipeModal-{{ $recipe->id }}')" class="px-4 py-2 bg-white dark:bg-stone-800 border border-slate-200 dark:border-stone-700 text-slate-700 dark:text-stone-300 text-sm font-medium rounded-xl hover:bg-slate-50 dark:hover:bg-stone-700 transition-colors">Cancelar</button>

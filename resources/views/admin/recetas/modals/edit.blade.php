    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-8">
        <div class="relative w-full max-w-lg bg-white shadow-2xl rounded-2xl">
            <div class="flex items-start justify-between px-6 py-4 border-b border-slate-100">
                <div>
                    <h3 class="text-xl font-bold text-slate-900">Editar Receta</h3>
                    <p class="text-sm text-slate-500 mt-1">REC-00{{ $recipe->id }}</p>
                </div>
                <button type="button" onclick="closeModal('editRecipeModal-{{ $recipe->id }}')" class="text-slate-400 hover:text-slate-600 transition-colors">

    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-8">
        <div class="relative w-full max-w-lg bg-white shadow-2xl rounded-2xl">
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                <h3 class="text-xl font-bold text-slate-900">Agregar Componente</h3>
                <button type="button" onclick="closeModal('addComponentModal')" class="text-slate-400 hover:text-slate-600 transition-colors">
            ...
            <form action="{{ route('recipes.components.store', $recipe->id) }}" method="POST">
                ...
                <div class="px-6 py-6 space-y-5">
                    <div>
                        ...
                        <input type="text" name="name" required placeholder="Ej: Tela denim 12oz" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-slate-700">
                    </div>
    ...
    <a href="{{ route('component-types.index') }}" target="_blank" class="text-[11px] text-emerald-600 hover:text-emerald-700 font-medium mt-1.5 inline-block">
        + Gestionar tipos de componente
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0.01" name="quantity" required placeholder="0" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none text-slate-700">
                    </div>
                </div>
                <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50 rounded-b-2xl">
                    <button type="button" onclick="closeModal('addComponentModal')" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50">Cancelar</button>
                    <button type="submit" class="px-5 py-2.5 text-sm font-semibold text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 shadow-sm shadow-emerald-600/30">Agregar</button>
                </div>
            </form>
        </div>
    </div>

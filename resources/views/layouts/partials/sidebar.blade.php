    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <nav class="space-y-1.5 px-3 pb-6">
            ...
            @if(Auth::user()->hasRole('admin') || Auth::user()->hasPermission('gestionar-usuarios'))
                <a href="{{ route('admin.users.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all relative {{ request()->routeIs('users.*') ? 'bg-blue-600 text-white shadow-md shadow-blue-900/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50' }}">
